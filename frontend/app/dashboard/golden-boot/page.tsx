      ...
      <div>
        ...
        <div className="mb-6">
          <label className="text-sm font-bold text-on-surface uppercase tracking-wider mb-3 block">
            Or Select Player from Dropdown
          </label>
          <select
          ...
          >
            ...
            {filteredPlayers.map((playerStat) => (
              <option key={playerStat.player.id} value={playerStat.player.id}>
                {playerStat.player.name} ({playerStat.player.team_code}) - {playerStat.predicted_goals?.toFixed(1)} predicted goals
              </option>
            ))}
          </select>
        </div>
            ...
            return (
              <div
                ...
                className={`relative overflow-hidden rounded-2xl p-6 cursor-pointer transition-all duration-300 ${
                  isSelected
                    ? 'bg-gradient-to-br from-primary/20 via-primary/10 to-transparent border-4 border-primary shadow-2xl shadow-primary/40 scale-105 ring-4 ring-primary/20'
                    : isMyPrediction
                    ? 'bg-gradient-to-br from-primary/10 to-transparent border-3 border-primary/60 shadow-xl'
                    : 'bg-gradient-to-br from-surface-container to-surface-container-high border-2 border-outline-variant hover:border-primary/60 hover:shadow-2xl hover:scale-102 hover:from-primary/5'
                }`}
              >
                {/* Background Pattern */}
                ...
                <div className="relative flex items-start justify-between mb-5 pb-4 border-b-2 border-gradient-to-r from-primary/40 via-primary/20 to-transparent">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className="px-3 py-1 bg-secondary/20 text-secondary rounded-lg font-bold text-sm border border-secondary/30">
                        {player.age} yrs
                      </span>
                    </div>
                ...
                <div className="relative grid grid-cols-2 gap-4 mb-5 pb-5 border-b border-outline-variant">
                  <div className="bg-gradient-to-br from-blue-500/20 to-blue-600/10 rounded-xl p-4 border-2 border-blue-500/30 shadow-lg">
                    <p className="text-xs text-blue-300 font-bold uppercase tracking-wider mb-2">Goals/90</p>
                    <p className="text-3xl font-black text-blue-400 drop-shadow-lg">
                      {player.goals_per_90?.toFixed(2) || 'N/A'}
                    </p>
                  </div>
                  <div className="bg-gradient-to-br from-purple-500/20 to-purple-600/10 rounded-xl p-4 border-2 border-purple-500/30 shadow-lg">
                    <p className="text-xs text-purple-300 font-bold uppercase tracking-wider mb-2">Shots/Game</p>
                ...
                {/* AI Prediction */}
                <div className="relative bg-gradient-to-br from-primary via-primary-fixed to-primary/80 rounded-2xl p-5 space-y-3 border-2 border-primary-container shadow-2xl">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-base text-white/90 font-bold">goals predicted</span>
                  </div>
                </div>

                {/* Selection Indicator */}
                {isSelected && (
                  <div className="relative mt-5 pt-5 border-t-2 border-primary/40">
                    <div className="flex items-center gap-3 text-primary text-base font-black justify-center animate-bounce">
